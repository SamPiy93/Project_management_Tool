<?php
    if(isset($this->session->userdata['USERTYPE'])){
        ...
        $userstype=($this->session->userdata['USERTYPE']);
        if($userstype != "admin"){
            echo "<script>alert(\"You are not authorized to visit this page\");window.location.href = '/login/index.php/person';</script>";    
        }
    }
    ...
    function loadData(){
            ...
            $('#hh').DataTable( {
                ...
                "columns": [
                    ...
                    {"data" : null,
                     "mRender": function(data, type, full) {
                         return '<button class="btn btn-primary  btn-animate btn-animate-side btn-block btn-sm" onclick="edit_person('+data.id+')"> Edit </button>' ;
                     }
                    },
                    {"data" : null,
                     "mRender": function(data, type, full) {
                    ...
                    {"data" : null,
                     "mRender": function(data, type, full) {
                         var stat = data.state;
                         var displayname = "Activate";
                         if(stat=="activate" || stat=="Activate"){
                          displayname = "Deactivate";
                         }
                         return '<button class="btn btn-danger  btn-animate btn-animate-side btn-block btn-sm" onclick="changestatus('+data.id+',\''+displayname+'\')"> '+displayname+' </button>' ;
                     }
                    }
                ]
            } );
